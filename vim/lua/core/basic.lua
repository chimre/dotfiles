...
vim.api.nvim_command('autocmd BufEnter * if expand("%") == "" | setlocal buftype=nofile | setlocal bufhidden=hide | setlocal noswapfile | endif')
vim.api.nvim_command([[autocmd BufEnter * if &filetype != '' && filereadable(g:vim_ftplugin . '\' . &filetype . '.vim') == 1 | call execute('source ' . g:vim_ftplugin . '\' . &filetype . '.vim') | endif]])
